...
if [ -n "$PIN" ]; then
  [[ "$PIN" =~ ^v[0-9]+\.[0-9]+\.[0-9]+(-beta\.[0-9]+)?$ ]] || die "--pin must be a release tag (vX.Y.Z or vX.Y.Z-beta.N)"
fi
...
if [ "$ENV_MODE" = prod ]; then
  [ "$REPO" = MartinKyng/Hera-Operating-System ] || die "production uses only the public release repository"
...
# --- Images (Docker Hub only; digests pinned by ${PIN:+--pin ${PIN}}${CHANNEL:+--channel ${CHANNEL}}${PIN:-${CHANNEL:-<none>}})
...
# Reject saved legacy/non-Hub image names before Compose can pull anything.
if [ "$ENV_MODE" = prod ]; then
  for key in HERA_API_IMAGE HERA_WEB_IMAGE; do
